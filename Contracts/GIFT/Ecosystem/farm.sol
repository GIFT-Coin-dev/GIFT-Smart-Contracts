...
contract GIFT is Context, IERC20, Ownable {
    ...
    string private constant NAME = "Gift Coin";
    string private constant SYMBOL = "GIFT";
    ...
    function setTaxPercentage(uint256 _taxPercentage) external onlyOwner {
        require(_taxPercentage >= 1 && _taxPercentage <= 5, "Value is outside of range 1-5");
...
contract GiftFarm is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of GIFT tokens
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. GIFT tokens to distribute per block.
        uint256 lastRewardBlock; // Last block number that GIFT tokens distribution occurs.
        uint256 accGiftPerShare; // Accumulated GIFT tokens per share, times 1e12. See below.
    }

    GIFT public immutable gift; // The GIFT ERC-20 Token.
    uint256 private giftPerBlock; // GIFT tokens distributed per block. Use getGiftPerBlock() to get the updated reward.

    PoolInfo[] public poolInfo; // Info of each pool.
    mapping(uint256 => mapping(address => UserInfo)) public userInfo; // Info of each user that stakes LP tokens.
    uint256 public totalAllocPoint; // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public startBlock; // The block number when GIFT token mining starts.

    uint256 public blockRewardUpdateCycle = 1 days; // The cycle in which the giftPerBlock gets updated.
    uint256 public blockRewardLastUpdateTime = block.timestamp; // The timestamp when the block giftPerBlock was last updated.
    uint256 public blocksPerDay = 28800; // The estimated number of mined blocks per day.
    uint256 public blockRewardPercentage = 1; // The percentage used for giftPerBlock calculation.
    ...
    constructor(
        ...
        require(address(_gift) != address(0), "GIFT address is invalid");
    ...
    modifier updateGiftPerBlock() {
        (uint256 blockReward, bool update) = getGiftPerBlock();
        if (update) {
            giftPerBlock = blockReward;
            blockRewardLastUpdateTime = block.timestamp;
        }
        _;
    }

    function getGiftPerBlock() public view returns (uint256, bool) {
        if (block.number < startBlock) {
            return (0, false);
        }

        uint256 poolReward = gift.balanceOf(address(this));
        if (poolReward == 0) {
            return (0, giftPerBlock != 0);
        }

        if (block.timestamp >= getGiftPerBlockUpdateTime() || giftPerBlock == 0) {
            return (poolReward.mul(blockRewardPercentage).div(100).div(blocksPerDay), true);
        }

        return (giftPerBlock, false);
    }

    function getGiftPerBlockUpdateTime() public view returns (uint256) {
    ...
    // Update the given pool's GIFT token allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending GIFT tokens on frontend.
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            (uint256 blockReward, ) = getGiftPerBlock();
            uint256 giftReward = multiplier.mul(blockReward).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    function updatePool(uint256 _pid) public updateGiftPerBlock {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        // no minting is required, the contract should have GIFT token balance pre-allocated
        // accumulated GIFT per share is stored multiplied by 10^12 to allow small 'fractional' values
        pool.accGiftPerShare = pool.accGiftPerShare.add(giftReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to GIFTFarming for GIFT token allocation.
    ...
    // Withdraw LP tokens from GIFTFarming
    ...
    // Safe GIFT token transfer function, just in case if
    // rounding error causes pool to not have enough GIFT tokens
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 balance = gift.balanceOf(address(this));
        uint256 amount = _amount > balance ? balance : _amount;
        gift.transfer(_to, amount);
    }
